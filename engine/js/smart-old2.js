...
const SmartFormatters = new function(){
    ...
    this.boldFormat = async function(){
            ...
            console.log("Last Content Bold Option Length: ", lastContent)
            //this last_content_length is what we are to use to 
            //if the length of the last content is last_content_length 
            ...
            smart_options_storage.last_content_length = last_content_length;
            smart_options_storage.last_content_captured = lastContent;
    ...
    smartMixer.load_dependencies().then( async (dependencies) => {
        ...
        smartContentArea.addEventListener("click", function(event){
            ...
            smartContentAreaWrapper.addEventListener("keyup", async function(event){
                ...
                console.log("Hey: ", online_content.trim().length);

                if(online_content.trim().length == 0){
                ...
                //set a global object that tracks which option is in use
                    ...
                    switch(formatter){
                        case "bold":
                            if(smart_options_storage.formatters.bold.option == true){
                                ...
                                //get the last content captured 
                                console.log("Last content captured from within using bold: ", smart_options_storage.last_content_captured)
                                last_content_captured = smart_options_storage.last_content_captured;
                                last_content_length_captured = smart_options_storage.last_content_length;

                                //substring
                                console.log("Lasr content length: ", last_content_length_captured)
                                console.log("Online content length: ", online_content.length);
                               
                                console.log("Lasr content: ", last_content_captured)
                                console.log("Online content : ", online_content );
                               
                                let content_to_bold;

                                if(last_content_length_captured > online_content.length){
                                    
                                    startIndex = last_content_length_captured - online_content.length;

                                    // content_to_bold = online_content.substring(startIndex)

                                    // console.log("Content to bold: ", content_to_bold)

                                    //check the last_content_length_captured for certain characters ..
                                    const regexSpaceStarts = /^&nbsp;/
                                    const regexSpaceEnds = /&nbsp;$/

                                    if(regexSpaceEnds.test(last_content_captured)){
                                        //space is present ..remove the space
                                        //console.log("Space ends it ..")

                                        last_content_captured_box = last_content_captured.split("&nbsp;")

                                        //console.log("Box: ", last_content_captured_box[0]);

                                        last_content_captured = last_content_captured_box[0];

                                        console.log("Last content captured: ", last_content_captured);
                                        console.log("Last content captured length: ", last_content_captured.length);

                                        content_to_bold = online_content.substring(last_content_captured.length + 1)

                                        console.log("content to bold: ", content_to_bold)

                                        content_to_bold = `<b>${content_to_bold}</b>`

                                        last_content_captured = last_content_captured + " ";

                                    }

                                    new_content = last_content_captured + content_to_bold;
                                    ...
                                    // content_to_bold = online_content.substring(last_content_length - 1)
                                    // console.log("Content to bold: ", content_to_bold)
                                    new_content = `<b>${online_content}</b>`;
                                   // new_content = online_content + content_to_bold
                                }
                                ...
                                //get the last content captured 
                                console.log("Last content captured from within not using bold: ", smart_options_storage.last_content_captured)
...
function caretHandlerEngine(current_tags_options, content){
    ...
    if(content_length <= 1){
        ...
        if(document.querySelector(".smart-content-area").childNodes[0].tagName == "B"){
            ...
            console.log("Nodes length: ", document.querySelector(".smart-content-area").childNodes.length)

            if(content_length > document.querySelector(".smart-content-area").childNodes.length){
                console.log(document.querySelector('.smart-content-area').innerText.length);  
                range_position.setStart(document.querySelector('.smart-content-area').childNodes[0],  document.querySelector(".smart-content-area").innerText.length)
